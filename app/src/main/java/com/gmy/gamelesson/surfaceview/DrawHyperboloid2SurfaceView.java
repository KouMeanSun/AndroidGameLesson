...
public class DrawHyperboloid2SurfaceView extends GLSurfaceView {
    private final float TOUCH_SCALE_FACTOR = 180.0f/320;//角度缩放比例
    SceneRenderer mRenderer;//场景渲染器
    private float mPreviousY;//上次的触控位置Y坐标
    private float mPreviousX;//上次的触控位置Y坐标

    public DrawHyperboloid2SurfaceView(Context context) {
        super(context);
    ...
    public class SceneRenderer implements GLSurfaceView.Renderer
    {
        ...
        public void onSurfaceCreated(GL10 gl, EGLConfig config) {
            ...
            cylinder=new DrawHyperboloid2(6f,2f,2f,2f,10,20f);
//            //开启一个线程自动旋转光源
            new Thread()
            {
          	  public void run()
