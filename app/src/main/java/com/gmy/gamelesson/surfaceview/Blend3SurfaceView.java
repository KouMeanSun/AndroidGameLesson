...
public class Blend3SurfaceView extends GLSurfaceView
{
    float cx=0;//摄像机x位置
    float cy=3;//摄像机y位置
    float cz=10;//摄像机z位置
    ...
    public Blend3SurfaceView(Context context) {
        super(context);
    ...
    private class SceneRenderer implements GLSurfaceView.Renderer
    {
        ...
        int floorId;//地板ID
        int coverId;//镜头ID
        Cube3 cubeTexture;//纹理立方体
        CubeColor2 cubeColor;//着色立方体
        TextureRect5 floor;//地板
        TextureRect5 cover;//镜头
        @Override
        ...
        public void onSurfaceCreated(GL10 gl, EGLConfig config) {
            ...
            cubeTextureId=initTexture(gl,R.mipmap.base);//初始化纹理
            floorId=initTexture(gl, R.mipmap.ground);
            coverId=initTexture(gl,R.mipmap.cover);
            cubeTexture=new Cube3(cubeTextureId,2,1,1);//创建纹理长方体
            cubeColor=new CubeColor2(1,2,2);//创建着色长方体
            floor=new TextureRect5//创建地板
                    (
                            1,0,1,floorId,
                            new float[]
                                    {
