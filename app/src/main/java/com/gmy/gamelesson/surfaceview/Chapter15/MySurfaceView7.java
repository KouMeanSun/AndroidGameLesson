...
import static com.gmy.gamelesson.surfaceview.Chapter15.Constant7.*;
...
public class MySurfaceView7 extends GLSurfaceView{
                ...
                for(int i=0;i<alBN.size();i++)
                {
                    ...
                    float[] bPosition=RotateUtil.calculateABPosition
                            (
                                    x, //触屏X坐标
                                    y, //触屏Y坐标
                                    Chapter15SenvenActivity.screenWidth, //屏幕宽度
                                    Chapter15SenvenActivity.screenHeight, //屏幕长度
                                    left, //视角left值
                                    top, //视角top值
                                    near, //视角near值
                                    far,
    ...
    private class SceneRenderer implements GLSurfaceView.Renderer
    {
        ...
        public void onSurfaceCreated(GL10 gl, EGLConfig config) {
            ...
            cubeId=initTexture(gl,R.mipmap.stone);//初始化立方体纹理ID
            textureId=initTexture(gl, R.mipmap.basketball);//初始化纹理ID
            textureZId=initTexture(gl,R.mipmap.robot);

            bnShape=new Cube7(cubeId,CUBE_X,CUBE_Y,CUBE_Z,-10f,0f,-4.0f);//创建立方体对象
            alBN.add(bnShape);
            bnShape=new Ball7(BALL_R,textureId,-5f,0,1.0f);//创建球
            alBN.add(bnShape);
            bnShape=new ZhuiTi7(textureZId,ZHUITI_X,ZHUITI_Y,ZHUITI_Z,-10f,0f,1.0f);//创建晶体
            alBN.add(bnShape);
            bnShape=new Cube7(cubeId,CUBE_X,CUBE_Y,CUBE_Z,-10f,0f,4.0f);
            ...
            bnShape=new Ball7(BALL_R,textureId,-5f,0,1.0f);//创建球
            alBN.add(bnShape);
            bnShape=new ZhuiTi7(textureZId,ZHUITI_X,ZHUITI_Y,ZHUITI_Z,8f,0f,1.0f);//创建晶体
            alBN.add(bnShape);
        }
