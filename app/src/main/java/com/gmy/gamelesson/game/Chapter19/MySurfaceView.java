...
import static com.gmy.gamelesson.game.Chapter19.Constant.*;
...
class MySurfaceView extends GLSurfaceView {
    ...
    class SceneRenderer implements GLSurfaceView.Renderer {	//创建场景渲染器类
        ...
        public void onSurfaceCreated(GL10 gl, EGLConfig config) {
            ...
            landTextureId=initTexture(gl,R.mipmap.grass);			//初始化陆地纹理
            skyTextureId=initTexture(gl, R.mipmap.skyball);		//初始化天空纹理
            ...
            int[] explodeImgId={R.mipmap.explode1,				//爆炸动画图片id
    				R.mipmap.explode2,R.mipmap.explode3,R.mipmap.explode4,
    				R.mipmap.explode5,R.mipmap.explode6};
            ...
            int heroMissileId=initTexture(gl,R.mipmap.heromissile);//炮弹纹理id
            int numberTexId=initTexture(gl,R.mipmap.number);		 //得分数字id
            
            planeCabinId = initTexture(gl,R.mipmap.planecabin);//机舱纹理
            planeHeadId = initTexture(gl,R.mipmap.planehead);	//hero机头纹理
            frontWingId = initTexture(gl,R.mipmap.frontwing);	//hero前机翼纹里
            frontWing2Id = initTexture(gl,R.mipmap.frontwing2);//hero前机翼纹里 2
            bacckWingId = initTexture(gl,R.mipmap.planebody);	//hero后机翼纹里
            topWingId = initTexture(gl,R.mipmap.topwing);
            planeBodyId = initTexture(gl,R.mipmap.planebody); //hero机身纹理
    		cylinder1Id= initTexture(gl,R.mipmap.yz1);		//hero圆柱1
    		cylinder2Id = initTexture(gl,R.mipmap.yz2);		//hero圆柱2
    		screw1Id = initTexture(gl,R.mipmap.planecabin);	//hero螺旋桨纹理id
    		
    		enemyPlaneFrontWingId = initTexture(gl,R.mipmap.ewing1);		//敌机前机翼纹里
    		enemyPlaneFrontWing2Id = initTexture(gl,R.mipmap.ewing2);		//敌机前机翼纹里2
    		enemyPlaneBodyId = initTexture(gl,R.mipmap.eplanebody); 		//敌机机身纹理
    		enemyPlaneTopWingId = initTexture(gl,R.mipmap.etopwing);
    		enemyPlaneHeadId =initTexture(gl,R.mipmap.eplanehead);		//敌机机头纹理
    		screw2Id = initTexture(gl,R.mipmap.etopwing);					//敌机螺旋桨纹理id
            ...
            int iconId=initTexture(gl,R.mipmap.icon);							//初始化图标纹理id
