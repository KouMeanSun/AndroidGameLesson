   ...
   { 
    ...
    {   
        ...
        public void onSurfaceCreated(GL10 gl, EGLConfig config) {
            ...
            //初始化图片纹理
            int baseId=initTexture(gl,R.mipmap.base);				//基台纹理图片
            int boxId=initTexture(gl, R.mipmap.box);				//箱子纹理图片
            int floorId=initTexture(gl,R.mipmap.floor);			//地面纹理图片
            int backgroundId=initTexture(gl,R.mipmap.background); //背景纹理图片
            int steelId=initTexture(gl,R.mipmap.line);			//绳子纹理图片
            int leafId=initTexture(gl,R.mipmap.leaf);				//树叶纹理图片
            int trunkId=initTexture(gl,R.mipmap.trunk);				//树干纹理图片
            int numberTexId=initTexture(gl,R.mipmap.number);		//得分数字纹理图片
            int buttonId=initTexture(gl,R.mipmap.okbutton);		//OK键纹理图片
